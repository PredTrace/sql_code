...
def compute_op(class_, *args):
    ...
    return Value(ret)
    #return ret
builtin_aggr_functions = ['sum','mean','median','std','avg','count','variance','corr','any','every','min','max']
...
operator_map = {Add:lambda x,y:x+y, Subtract:lambda x,y:x-y, Multiply: lambda x,y:x*y, Divide: lambda x,y:x/y,\
               ...
               If: lambda x,y,z:y if x else z, Length:lambda x:len(x), In: lambda x,*y: z3.Or(*[x==y_ for y_ in y])}
...
def compute_aggregate(expr, table):
    ...
    for tup in table:
        ...
        rs = z3.If(tup.eval_exist_cond(), getv(v), rs)
...
def get_filter_replacing_field(expr, column_to_replace):
    if expr is None or is_constant_expr(expr):
        return expr
    if isinstance(expr, UnresolvedAttribute):
        ...
        return UnresolvedFunction(expr.name, [get_filter_replacing_field(e, column_to_replace) for e in expr.children], expr.isDistinct)
    elif isinstance(expr, Alias):
        return Alias(get_filter_replacing_field(expr.child, column_to_replace),expr.name)
    elif isinstance(expr, UnaryExpression):
        return expr.__class__(get_filter_replacing_field(expr.child, column_to_replace))
    elif isinstance(expr, BinaryExpression):
        return expr.__class__(get_filter_replacing_field(expr.left, column_to_replace),\
                              get_filter_replacing_field(expr.right, column_to_replace))
    elif isinstance(expr, In):
        return In(get_filter_replacing_field(expr.value, column_to_replace), [get_filter_replacing_field(e, column_to_replace) for e in expr.list])
        ...
        return CaseWhen([(get_filter_replacing_field(b[0], column_to_replace), get_filter_replacing_field(b[1], column_to_replace)) for b in expr.branches],\
