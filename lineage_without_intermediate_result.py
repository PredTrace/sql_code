...
def obtain_predicate_candidate_pushup(op, preds):
    if isinstance(op, FilterInference):
        ...
        ret = get_filter_replacing_nonexist_column(preds[0], col_used)
        op.input_filters = [ret]
        return ret
...
def lineage_inference_no_intermediate_result(ppl, output_filter):
    ...
    while True:
        ...
        while len(queue) > 0:
            ...
            if no_intermediate_result==False:
                ...
                if len(ppl_node.upstream_nodes) > 1:
                    ...
                    columns_used = [get_columns_used(ppl_node.inference.input_filters[i]) for i in range(len(ppl_node.inference.input_filters))]
                    consts = [[str(p[1].value) for p in get_equivalent_pairs(pred) if isinstance(p[1], Literal)] for pred in ppl_node.inference.input_filters]
                    # processed_columns = set()
                    # for n in readtable_nodes:
                    #     if n.processing_order > 0:
                    #         processed_columns = processed_columns.union(set([col_name.split('.')[-1] for col_name,col_type in n.output_schema]))
                    def custom_score(column_used, const):
                        # return 100 if any(['r_name' in c for c in column_used]) \
                        #     else 0
                        return len(const)
                            #sum([5 if 'key' in c else 0 for c in column_used ])
                    temp = [(i, (temp0[i][1]+custom_score(columns_used[i], consts[i])) \
                                        if not all([source in table_filters for source in node_source_map[ppl_node.upstream_nodes[i]]]) else -1)  for i in range(0, len(ppl_node.inference.input_filters))]
                    
                    max_fields = max([x[1] for x in temp])
                    keep = 0
                    #print("consts = {}, temp = {}, max_fields = {}".format([len(x) for x in consts], temp, max_fields))
        ...
        while len(queue) > 0:
            ppl_node = queue.pop(0)
            preds_to_push = simplify_pred(processed_pushup_nodes[ppl_node])
