...
def obtain_predicate_candidate(op, pred):
    ...
    if isinstance(op, FilterInference):
        ...
        # print(" $ $ pred to add left = {}".format(','.join([str(x) for x in pred_to_add_left])))
        # print(" $ $ pred to add right = {}".format(','.join([str(x) for x in pred_to_add_right])))
...
def obtain_subquery_filter(op, pred):
    ...
    if isinstance(op.subquery_cmp, InSubquery) or isinstance(op, EqualTo):
        ...
        if isinstance(field, UnresolvedAttribute) and is_col_in_exist_columns(field.get_full_name(), [p1.get_full_name() for p1,p2 in equiv_pairs]):
            target_cmp = list(filter(lambda x: is_col_in_exist_columns(field.get_column_name(), [x[0].get_full_name()]), equiv_pairs))[0]
            pred_to_push = EqualTo(wrap_column_name(op.subppl.output_schema[0][0]), target_cmp[1])
    ...
    def replace_variable_in_correlated_subquery(ppl_node):
        if isinstance(ppl_node.inference, FilterInference):
            ppl_node.inference.condition = get_filter_replacing_field(ppl_node.plan_node.condition, field_to_replace)
    ...
    #print("Predicate push through subpip = {}".format(expr_to_sql(pred_to_push)))
    return pred_to_push
    # print("+ + subpipeline")
...
def predicate_pushdown_pipeline(ppl, output_filter, can_store_intermediate_result=True): # ppl: the last node
    ...
    while len(queue) > 0:
        ...
        if no_intermediate_result==False and can_store_intermediate_result:
            ...
            pred_to_push = generate_row_selection_predicate(opt_schema)#generate_row_selection_predicate(ppl_node.output_schema)
